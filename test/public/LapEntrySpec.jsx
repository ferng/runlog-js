...
import LapEntry from '../../public/LapEntry';
...
test('LapEntry is well formed', (t) => {
  ...
  const wrapper = Enzyme.mount(<LapEntry time={cleanLap.time} distance={cleanLap.distance} unit={cleanLap.unit} />, {
    context,
    childContextTypes: {
...
test('changes to time and distance update the state but do not trigger an onChange', (t) => {
  ...
  const context = { refData: getRefData() };
  const wrapper = Enzyme.mount(<LapEntry time={testLap1.time} distance={testLap1.distance} unit={testLap1.unit} onChange={onChange} />, {
    context,
    childContextTypes: {
      multipliers: PropTypes.object,
      refData: PropTypes.array,
...
test('changes to units update the state and an onChange', (t) => {
  ...
  t.equal(instance.state.unit, testLap1.unit);
  // make sure state is updated
  wrapper.find('#unit').first().simulate('change', { target: { id: 'unit', value: testLap2.unit } });
